...
class DatasetProcessor:
    ...
    def __init__(self, args, bcq=False):
        self.args = args
        self.bcq = bcq
    ...
    def get_dataset(self, is_render_traj=False, done_frame=True):
        ...
        if 'antmaze' in env_name:
            traj_list = dataset_utils.processed_sequence_dataset(self.args, env, env_name, timeout_frame=False, done_frame=True, bcq=self.bcq)
        else:
            traj_list = dataset_utils.processed_sequence_dataset(self.args, env, env_name, timeout_frame=False, done_frame=done_frame, bcq=self.bcq)
