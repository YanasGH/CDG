...
def render_BCQ(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    # For saving files
    setting = f"CARI_{args.env_name}_{args.seed}"
    buffer_name = f"CARI_{args.buffer_name}_{setting}"
    ...
    for time in range(video_episodes):
        ...
        save_frames_as_gif(abstract, path='./videos/', filename='CARI_{}_{}_{}.gif'.format(args.env_name, time, int(avg_reward)))
...
def train_BCQ(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    # For saving files
    setting = f"CARI_{args.env_name}_{args.seed}_{args.entropy_weight}"
    buffer_name = f"CARI_{args.buffer_name}_{setting}"
    ...
    if args.train_bcq:
        ...
        rew_mean = eval_policy(policy, args.env_name, args.seed, training_iters, args, 100)  # 10000 to make sure the evaluation is as good of an approximation as possible
...
def render_traj(args):
    ...
    save_frames_as_gif(abstract, path='./videos/',
                       filename='CARI_{}_{}_{}.gif'.format(args.env_name, "render_traj", time.time()))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_stamp", default="_0.5_210000.0", #default="_205000.0",
                        type=str)  # load_stamp
    ...
    parser.add_argument("--is_forward_rollout", action="store_true")  # Change to "store_false" if you want to train FOMI+BCQ
    ...
    parser.add_argument('--demonstration', default=True, type=bool)  # set to True if the training is meant for demonstration
    ...
    if args.task_name[:7] == 'maze2d-' or args.task_name[:8] == 'antmaze-' or \
        ...
        args.forward_model_load_path = 'mopo_models/' + args.task_name + '_CARI_forward_{}/'.format(args.seed)
        args.reverse_model_load_path = 'mopo_models/' + args.task_name + '_CARI_reverse_{}/'.format(args.seed)
    else:
    ...
    env = gym.make(args.env_name)  # env_name because it is used in fake_env for model rollouts
