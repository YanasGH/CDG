...
def train_reverse_bc(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    # For saving files
    setting = "CARI_{}_{}_{}".format(args.env_name, args.seed, args.entropy_weight)
    buffer_name = "CARI_{}_{}".format(args.buffer_name, setting)


    # Initialize policy
    if not args.is_forward_rollout:
        print("---------------------------------")
        print("Training reverse BC.")
        print("---------------------------------")
        policy = ReverseBC.ReverseBC(state_dim, action_dim, max_action, device, args.entropy_weight)
    else:
        print("---------------------------------")
        print("Training forward BC.")
        print("---------------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_freq",  default=5e1,  # for demonstrations, else use 5e4
                        type=float)  # How often (time steps) we evaluate
    parser.add_argument("--max_timesteps",  default=5e1,  # for demonstrations, else use 5e5
    ...
    parser.add_argument("--is_forward_rollout", action="store_true") # Change to "store_false" if you want to train FBC
    ...
    if args.task_name[:7] == 'hopper-' or args.task_name[:9] == 'walker2d-':
        ...
        print(os.getcwd(), args.reverse_model_load_path)
    else:
