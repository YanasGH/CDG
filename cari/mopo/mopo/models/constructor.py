...
from mopo.mopo.models.fc import FC
from mopo.mopo.models.bnn import BNN
...
def construct_model(obs_dim=11, act_dim=3, rew_dim=1, hidden_dim=200, num_networks=7,
	...
	if not model.model_loaded:
		if model_type == 'identity':
			...
			model.add(FC(obs_dim+rew_dim, input_dim=hidden_dim+hidden_dim, weight_decay=0.0001))

			#Context layer
			model.add(FC(hidden_dim, input_dim=hidden_dim, weight_decay=0.0001), context_layer=True)

			if separate_mean_var:
				model.add(FC(obs_dim+rew_dim, input_dim=hidden_dim+hidden_dim, weight_decay=0.0001), var_layer=True)
...
def format_reverse_samples_for_training(samples):
	obs = samples['observations']
	act = samples['actions']
	next_obs = samples['next_observations']
	rew = samples['rewards']
