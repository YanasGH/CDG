...
class VAE(torch.nn.Module):
    ...
    def encode(self, state, action):
        ...
        mu, logstd = torch.chunk(self.encoder(state_action.float()), 2, dim=-1)
    ...
    def decode(self, state, z=None):
        ...
        action = self.decoder(torch.cat([state.float(), z.float()], dim=-1))
...
class AlgoTrainer(BaseAlgo):
        ...
        for epoch in range(self.args['max_epoch']):
            for i in range(self.args['steps_per_epoch']):
                ...
                with torch.no_grad():
                    ...
                    obs_action = torch.cat([repeat_next_obs, multiple_actions], dim=-1).float()
                ...
                obs_action = torch.cat([obs, action], dim=-1).float()
                q1 = self.q1(obs_action).float()
                q2 = self.q2(obs_action).float()
            ...
            print(f"Epoch {epoch}, RESULT: {res}")
    ...
    def eval(self, callback_fn):
        mean = 0
        std = 1.
        normalize = False
        res = callback_fn(self.get_policy(), None, mean, std, normalize)
        print(f"D4RL Score: {d4rl_score(self.task, res['Reward_Mean_Env'])}")
        self.writer.add_scalar('test return', res['Reward_Mean_Env'])
        self.writer.add_scalar('score', d4rl_score(self.task, res['Reward_Mean_Env']))
