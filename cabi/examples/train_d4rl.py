...
def run_algo(**kwargs):
    ...
    train_buffer, weights = load_d4rl_buffer(algo_config["task"], kwargs['data_path'], kwargs['isMediumExpert'], kwargs['data_proportion'], algo_config['use_per'])
    ...
    if algo_config['use_cabi']:
        ...
        print("--------------------")
        algo_trainer.eval(callback_fn=callback)
    else:
        callback.initialize(train_buffer=train_buffer, val_buffer=None, task=algo_config["task"])
