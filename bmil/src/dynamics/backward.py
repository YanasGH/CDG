...
class BackwardModel(torch.nn.Module):
    def __init__(
        ...
        if action_space is not None:
            self.act_low = action_space.low[0]  # torch.from_numpy(action_space.low).float().to(device)
            self.act_high = action_space.high[0]  # torch.from_numpy(action_space.high).float().to(device)
        if observation_space is not None:
            self.obs_low = observation_space.low[0]  # torch.from_numpy(observation_space.low).float().to(device)
            self.obs_high = observation_space.high[0]  # torch.from_numpy(observation_space.high).float().to(device)
