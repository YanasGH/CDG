...
class VAE(nn.Module):
    ...
    def decode(self, state, z=None):
        ...
        a = F.relu(self.d1(torch.cat([state.float(), z.float()], 1)))
        a = F.relu(self.d2(a))
...
class AlgoTrainer(BaseAlgo):
    ...
    def _train_forward_vae(self, data, fvae, fvae_optim):
        data.to_torch(device=self.device)
        state = data['obs'].float()
        action = data['act'].float()
        ...
        fvae_loss.float().backward()
        fvae_optim.step()
    
    def _train_backward_vae(self, data, bvae, bvae_optim):
        data.to_torch(device=self.device)
        next_state = data['obs_next'].float()
        action = data['act'].float()
        # Variational Auto-Encoder Training
