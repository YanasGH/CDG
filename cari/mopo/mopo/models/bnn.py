...
from mopo.mopo.models.utils import get_required_argument, TensorStandardScaler
from mopo.mopo.models.fc import FC
from mopo.mopo.utils.logging import Progress, Silent
...
class BNN:
    ...
    def add(self, layer, var_layer=False, context_layer=False):
        ...
        if not var_layer and not context_layer:
            if len(self.layers) > 0 and layer.get_input_dim() is None:
            ...
            if var_layer:
                ...
                if context_layer:
                    if len(self.context_layers) > 0:
                        layer.set_input_dim(self.context_layers[-1].get_output_dim())
                    self.context_layers.append(layer.copy())
    ...
    def pop(self, var_layer=False):
        ...
        """
        # TODO: add the popping of a context layer
        if not var_layer:
            ...
            with self.sess.as_default():
                with tf.compat.v1.variable_scope(self.name):
                    ...
                    self.min_logvar = tf.Variable(-np.ones([1, self.var_layers[-1].get_output_dim()])*10., dtype=tf.float32,
                                                  name="min_log_var")
                    for i, layer in enumerate(self.layers + self.context_layers):
    ...
    def _save_state(self, idx):
        ...
        if len(self.context_layers)>0:
            self._state[idx].extend([layer.get_model_vars(idx, self.sess) for layer in self.context_layers])
        if self.separate_mean_var:
    ...
    def _set_state(self):
        ...
        if len(self.context_layers) > 0:
            num_layers = len(self.context_layers)
            for layer in range(num_layers):
                params = {key: np.stack([self._state[net][len(self.layers) + layer][key] for net in range(self.num_nets)]) for key in keys}
                ops.extend(self.context_layers[layer].set_model_vars(params))

        #Last (output) layer
        params = {key: np.stack([self._state[net][len(self.layers)-1][key] for net in range(self.num_nets)]) for key in keys}
        ops.extend(self.layers[len(self.layers)-1].set_model_vars(params))

        if self.separate_mean_var:
            num_layers = len(self.var_layers)
            for layer in range(num_layers):
                # net_state = self._state[i]
                params = {key: np.stack([self._state[net][len(self.layers)+len(self.context_layers)+layer][key] for net in range(self.num_nets)]) for key in keys}
    ...
    def reset(self):
        ...
        if len(self.context_layers) > 0:
            [layer.reset(self.sess) for layer in self.context_layers]
    ...
    def load(self, inputs, targets, holdout_ratio=0.0, max_logging=1000, timer=None):
    ...
    def _load_structure(self):
        ...
        self.context_layers = structure[-1]
    ...
    def _compile_outputs(self, inputs, ret_log_var=False):
        ...
        if not self.separate_mean_var:
            ...
            #Just one context layer so should be fine
            cur_out_context = self.context_layers[0].compute_output_tensor(cur_out)
            cur_out = self.layers[-1].compute_output_tensor(values=tf.concat([cur_out, cur_out_context], axis=2))
            ...
            #Just one context layer
            context = self.context_layers[0].compute_output_tensor(mean)
            mean = tf.concat(values = [mean, context], axis=2)
            means.append(mean)
            mean = self.layers[-1].compute_output_tensor(mean)
            means.append(mean)
